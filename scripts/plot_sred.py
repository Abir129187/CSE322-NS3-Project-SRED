...
def run_simulations():
    flows_symmetrical = [10]

    # Symmetrical Runs
    for flows in flows_symmetrical:
        print(f"Running symmetrical topology simulation with {flows} flows...")
        cmd = f'./ns3 run "sred-paper-sim --nFlows={flows} --topologyType=symmetrical --simTime={SIM_TIME}"'
        subprocess.run(cmd, shell=True)

    # # Asymmetrical Run
    # print("Running asymmetrical topology simulation with 100 flows...")
    # cmd = f'./ns3 run "sred-paper-sim --nFlows=100 --topologyType=asymmetrical --simTime={SIM_TIME}"'
    # subprocess.run(cmd, shell=True)
